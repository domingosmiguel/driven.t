...
describe('GET /hotels', () => {
  ...
  describe('when token is valid', () => {
    ...
    it('should respond with status 404 when doesnt have hotel', async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
...
describe('GET /hotels/:hotelId', () => {
  it('should respond with status 401 if no token is given', async () => {
    const response = await server.post(`/hotels/${faker.random.numeric(5)}`);
  ...
  it('should respond with status 401 if given token is not valid', async () => {
    ...
    const response = await server.post(`/hotels/${faker.random.numeric(5)}`).set('Authorization', `Bearer ${token}`);
  ...
  describe('when token is valid', () => {
    ...
    it('should respond with status 404 when doesnt have hotel', async () => {
      ...
      const response = await server.get(`/hotels/${faker.random.numeric(5)}`).set('Authorization', `Bearer ${token}`);
